...
async function test() {
  ...
  // Create another message right away
  const envelope2 = proteus.session.Session.encrypt(aliceSession, "hello2");
  ...
  console.log('Bob decrypted 1:', new TextDecoder().decode(plaintextBytes));

  const plaintextBytes2 = await bobSession.decrypt(bobStore, envelope2);
  console.log('Bob decrypted 2:', new TextDecoder().decode(plaintextBytes2));
}
