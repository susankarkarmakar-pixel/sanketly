...
async function test() {
  ...
  // Try serializing and deserializing Alice's session before decrypting
  const aliceSessionBuf = aliceSession.serialise();
  const aliceSessionRestored = proteus.session.Session.deserialise(aliceId, aliceSessionBuf);

  const envelope2 = proteus.session.Session.encrypt(bobSession, "world");

  const pt2 = await aliceSessionRestored.decrypt(new MockPreKeyStore(), envelope2);
